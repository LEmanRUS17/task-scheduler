...
final class AttachWorkflowToTeamControllerTest extends WebTestCase
{
    ...
    public function testReturnsUnauthorizedWhenNotAuthenticated(): void
    {
        ...
        $client->request('POST', '/workflows/wf-1/teams/team-1');
    ...
    public function testReturnsForbiddenWhenCallerIsNotATeamMember(): void
    {
        ...
        $client->request('POST', '/workflows/wf-1/teams/team-1', server: [
    ...
    public function testAttachesWorkflowWhenCallerIsATeamMember(): void
    {
        $user = $this->makeUser();
        $client = static::createClient();
        $this->stubUserRepository($user);
    ...
    public function testReturnsConflictWhenServiceRejectsAttachment(): void
    {
        $user = $this->makeUser();
        $client = static::createClient();
        $this->stubUserRepository($user);
        ...
        $workflowService->method('attachToTeam')->willThrowException(
            new \DomainException('Only the workflow owner can attach it to a team'),
        );
        static::getContainer()->set(WorkflowServiceInterface::class, $workflowService);
